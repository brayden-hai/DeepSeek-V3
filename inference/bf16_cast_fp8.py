...
"""
NOTE: this will only plant weights into the new folder.
# I copied files in this way.
src=/fsx/training/output/neha/u13_dsv31_deepseekv31_4e-4_2025_09_12_05_36/merged-18
dst=/fsx/training/output/neha/modelopt-checkpoints/u13_dsv31_deepseekv31_4e-4_2025_09_12_05_36_quantized-fp8-pb-wo-18

for f in generation_config.json tokenizer.json tokenizer_config.json special_tokens_map.json configuration_deepseek.py modeling_deepseek.py; do
  if [ -f "$src/$f" ]; then cp -n "$src/$f" "$dst/"; fi
done
IMPORTANT: For config.json you should manually copy the config from the original checkpoint here: https://huggingface.co/deepseek-ai/DeepSeek-V3.1
"""
